...
define([
    ...
    var loadScript = function(src, test, callback) {
        ...
        var existing = document.querySelector(
            'script[data-videonotes-src="' + src + '"]'
        );
    ...
    YouTubePlayer.prototype.getCurrentTime = function() {
        return Promise.resolve(
            this.yt && this.yt.getCurrentTime
                ? this.yt.getCurrentTime()
                : 0
        );
    };
    ...
    var VimeoPlayer = function(container, url) {
        ...
        loadScript(
            ...
            function() {
                self.vm = new window.Vimeo.Player(container, {
                    url: url,
                    responsive: true,
                    dnt: true
                });
    ...
    VimeoPlayer.prototype.seek = function(t) {
        return this.vm
            ? this.vm.setCurrentTime(Math.max(0, t))
            : Promise.resolve();
    };

    var makePlayer = function() {
        var c = document.getElementById('videonotes-player');
        var p = config.player;
    ...
    var updateProgress = function(data) {
        ...
        var label = document.getElementById('videonotes-percent');
        var bar = document.getElementById('videonotes-progress-bar');
        ...
        try {
            config.segments =
                JSON.parse(data.segments) || config.segments;
    ...
    var tracker = function() {
        ...
        var flush = function() {
            ...
            return Ajax.call([{
                methodname: 'mod_videonotes_update_progress',
                args: args
            }])[0]
                .then(function(r) {
                    updateProgress(r);
        ...
        var tick = function() {
            ...
            Promise.all([
                ...
                var now = Date.now();
                var cur = Number(v[0]) || 0;
                var dur = Number(v[1]) || 0;
    ...
    var editor = function(note) {
        ...
        document.getElementById(
            'videonotes-noteid'
        ).value = note ? note.id : 0;

        document.getElementById(
            'videonotes-timecode'
        ).value = note ? note.timecode : 0;

        document.getElementById(
            'videonotes-editor-time'
        ).textContent = fmt(
            note ? note.timecode : 0
        );
    ...
    var updateEmpty = function() {
        var notes = document.querySelectorAll(
            '#videonotes-list .videonotes-note'
        ).length;
    ...
    var saveNote = function() {
        var shared = document.getElementById(
            'videonotes-shared'
        );
        ...
        Ajax.call([{
            methodname: 'mod_videonotes_save_note',
            args: args
        }])[0]
            .then(function(n) {
                return renderNote(n);
    ...
    var drawTimeline = function() {
        ...
        line.querySelectorAll(
            '.videonotes-marker,.videonotes-watched'
        ).forEach(function(n) {
            n.remove();
        });
            ...
            function(e) {
                ...
                if (edit) {
                    editor({
                        ...
                        shared:
                            edit.dataset.shared === 'true' ||
                            edit.dataset.shared === '1'
                    });

                    return;
                }

                var del = e.target.closest(
                    '.videonotes-delete'
                );
