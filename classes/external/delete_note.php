...
class delete_note extends external_api {
    ...
    public static function execute(int $cmid, int $noteid): array {
        ...
        $params = self::validate_parameters(self::execute_parameters(), ['cmid' => $cmid, 'noteid' => $noteid]);
        ...
        (new note_manager())->delete((int)$activity->id, (int)$USER->id, (int)$params['noteid']);
        $count = $DB->count_records('videonotes_notes', ['videonotesid' => $activity->id, 'userid' => $USER->id]);
        ...
        return ['deleted' => true, 'notecount' => $count];
    }
